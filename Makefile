...
# Regenerate the sqlc schema snapshot from ordered runtime migrations.
# Runtime migrations remain the source of truth; this file is only for sqlc.
db-schema-generate:
	...
	{ \
		...
		for f in $$(find internal/db/migrations -type f -name '*.up.sql' | LC_ALL=C sort); do \
			printf '\n-- %s\n' "$$f"; \
			cat "$$f"; \
			printf '\n'; \
		done; \
	} > "$$tmp"; \
	awk 'BEGIN { last_nonblank = 0 } { lines[NR] = $$0; if ($$0 != "") last_nonblank = NR } END { for (i = 1; i <= last_nonblank; i++) print lines[i] }' "$$tmp" > internal/db/sqlc/schema.sql; \
	rm -f "$$tmp"
